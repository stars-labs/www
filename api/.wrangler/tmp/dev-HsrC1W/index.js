...
// .wrangler/tmp/bundle-x6F5Yi/checked-fetch.js
...
__export(schema_exports, {
  ...
  STARS_DECIMALS: () => STARS_DECIMALS,
  STARS_TO_STARSHARS: () => STARS_TO_STARSHARS,
  blocks: () => blocks,
  botConfig: () => botConfig,
...
var wallets = sqliteTable("wallets", {
  ...
  // Balance in starshars (string for precision)
...
var blocks = sqliteTable("blocks", {
  ...
  // Mining nonce (large number as string)
  minerAddress: text("miner_address").notNull(),
  // Miner's wallet address
  reward: text("reward").notNull(),
  // Block reward in starshars
  txCount: integer("tx_count").notNull().default(0),
  // Number of transactions
  gasUsed: text("gas_used").default("0"),
...
var mempool = sqliteTable("mempool", {
...
var miningJobs = sqliteTable("mining_jobs", {
...
var botConfig = sqliteTable("bot_config", {
  ...
  lastTxAt: integer("last_tx_at", { mode: "timestamp" }),
  // Last transaction time
  txCount: integer("tx_count").notNull().default(0),
  // Total transactions sent
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`CURRENT_TIMESTAMP`)
});
var chainState = sqliteTable("chain_state", {
  id: integer("id").primaryKey().default(1),
  // Always 1
  ...
  currentDifficulty: integer("current_difficulty").notNull().default(4),
  // Current mining difficulty
  nextDifficultyAdjust: integer("next_difficulty_adjust").notNull().default(10),
  // Block height for next adjustment
  averageBlockTime: integer("average_block_time").default(5e3),
  // Average time in ms
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().default(sql`CURRENT_TIMESTAMP`)
});
var STARS_DECIMALS = 10;
var STARS_TO_STARSHARS = BigInt(10 ** STARS_DECIMALS);
var MINING_REWARD = (1n * STARS_TO_STARSHARS).toString();
var MIN_TX_FEE = "1";
var GENESIS_SUPPLY = (1000000n * STARS_TO_STARSHARS).toString();
...
var app = new Hono2();
var createBlockSchema = external_exports.object({
  hash: external_exports.string().min(1),
  previousHash: external_exports.string().min(1),
  height: external_exports.number().int().min(0),
  chainId: external_exports.string().min(1),
  transactionCount: external_exports.number().int().min(0).optional(),
  minerAddress: external_exports.string().optional(),
  difficulty: external_exports.number().optional(),
  nonce: external_exports.number().int().optional()
});
app.get("/blocks", async (c) => {
  ...
  try {
    const query = chainId ? db.select().from(blocks).where(eq(blocks.chainId, chainId)) : db.select().from(blocks);
    const result = await query.orderBy(desc(blocks.height)).limit(limit).offset(offset);
    return c.json({
      blocks: result,
      count: result.length,
      limit,
...
app.get("/stats", async (c) => {
  const db = c.get("db");
  try {
    const totalBlocks = await db.select({ count: sql`count(*)` }).from(blocks);
    const chains = await db.select({
      chainId: blocks.chainId,
      blockCount: sql`count(*)`,
      maxHeight: sql`max(${blocks.height})`
    }).from(blocks).groupBy(blocks.chainId);
    const recentBlocks = await db.select().from(blocks).orderBy(desc(blocks.timestamp)).limit(5);
    return c.json({
...
var blockchain_default = app;

// src/routes/transactions.ts
var app2 = new Hono2();
var createTransactionSchema = external_exports.object({
  hash: external_exports.string().min(1),
  blockHash: external_exports.string().optional(),
  fromAddress: external_exports.string().min(1),
  toAddress: external_exports.string().min(1),
  value: external_exports.number().positive(),
  fee: external_exports.number().optional(),
  status: external_exports.enum(["pending", "confirmed", "failed"]).optional(),
  userCreated: external_exports.boolean().optional()
});
...
app2.get("/", async (c) => {
  ...
  try {
    ...
    if (userCreatedParam !== void 0) {
      conditions.push(eq(transactions.userCreated, userCreated));
    }
    if (conditions.length > 0) {
      query = query.where(and(...conditions));
    }
    const result = await query.orderBy(desc(transactions.id)).limit(limit).offset(offset);
    return c.json({
      transactions: result,
      count: result.length,
      limit,
...
app2.get("/:hash", async (c) => {
  const db = c.get("db");
  const hash = c.req.param("hash");
  try {
    const [transaction] = await db.select().from(transactions).where(eq(transactions.hash, hash)).limit(1);
    if (!transaction) {
...
app2.post("/", zValidator("json", createTransactionSchema), async (c) => {
  const db = c.get("db");
  const data = c.req.valid("json");
  try {
...
app2.patch("/:hash", zValidator("json", updateTransactionSchema), async (c) => {
  const db = c.get("db");
  const hash = c.req.param("hash");
  const data = c.req.valid("json");
  try {
    const [updated] = await db.update(transactions).set(data).where(eq(transactions.hash, hash)).returning();
    if (!updated) {
      return c.json({ error: "Transaction not found" }, 404);
    }
...
app2.get("/stats/summary", async (c) => {
  const db = c.get("db");
  try {
    const stats = await db.select({
      ...
      failedCount: sql`sum(case when status = 'failed' then 1 else 0 end)`,
      userCreatedCount: sql`sum(case when user_created = true then 1 else 0 end)`,
      totalValue: sql`sum(value)`,
      totalFees: sql`sum(fee)`
    }).from(transactions);
...
app2.get("/mempool/pending", async (c) => {
  const db = c.get("db");
  try {
    const pendingTxs = await db.select().from(transactions).where(eq(transactions.status, "pending")).orderBy(desc(transactions.id)).limit(50);
    return c.json({
      mempool: pendingTxs,
...
function calculateTarget(difficulty) {
  ...
  const target = maxTarget >> BigInt(difficulty * 4);
  return "0x" + target.toString(16).padStart(64, "0");
}
__name(calculateTarget, "calculateTarget");
...
__name(meetsTarget, "meetsTarget");
function createBlockHeader(template) {
  return [
...
__name(createBlockHeader, "createBlockHeader");
function hashBlock(header, nonce) {
  const data = header + ":" + nonce;
  const hash = sha2562(data);
  return "0x" + bytesToHex(hash);
}
__name(hashBlock, "hashBlock");
function verifyPoW(blockHeader, nonce, target) {
...
function calculateMerkleRoot(txHashes) {
  ...
  while (level.length > 1) {
    ...
    for (let i = 0; i < level.length; i += 2) {
      ...
      const hash = bytesToHex(sha2562(combined));
      nextLevel.push(hash);
    }
    level = nextLevel;
  }
...
__name(calculateMerkleRoot, "calculateMerkleRoot");
function adjustDifficulty(currentDifficulty, actualTime, targetTime) {
  ...
  return Math.max(1, Math.min(32, newDifficulty));
}
__name(adjustDifficulty, "adjustDifficulty");
...
var genBase58 = /* @__PURE__ */ __name(/* @__NO_SIDE_EFFECTS__ */ (abc) => /* @__PURE__ */ chain(/* @__PURE__ */ radix(58), /* @__PURE__ */ alphabet(abc), /* @__PURE__ */ join("")), "genBase58");
var base58 = /* @__PURE__ */ genBase58("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz");
var createBase58check = /* @__PURE__ */ __name((sha2563) => /* @__PURE__ */ chain(checksum(4, (data) => sha2563(sha2563(data))), base58), "createBase58check");

// node_modules/@scure/bip39/index.js
var isJapanese = /* @__PURE__ */ __name((wordlist2) => wordlist2[0] === "\u3042\u3044\u3053\u304F\u3057\u3093", "isJapanese");
function nfkd(str) {
  if (typeof str !== "string")
...
var psalt = /* @__PURE__ */ __name((passphrase) => nfkd("mnemonic" + passphrase), "psalt");
...
function createHmacDrbg(hashLen, qByteLen, hmacFn) {
  ...
  const u8n2 = /* @__PURE__ */ __name((len) => new Uint8Array(len), "u8n");
  ...
  const reseed = /* @__PURE__ */ __name((seed = NULL2) => {
    k = h(byte02, seed);
    v = h();
    if (seed.length === 0)
      return;
...
function createKeygen(randomSecretKey2, getPublicKey2) {
  return /* @__PURE__ */ __name(function keygen2(seed) {
...
function weierstrass(params, extraOpts = {}) {
  ...
  class Point3 {
    ...
    static fromBytes(bytes) {
      const P2 = Point3.fromAffine(decodePoint(abytes(bytes, void 0, "point")));
      P2.assertValidity();
      return P2;
    }
    static fromHex(hex) {
      return Point3.fromBytes(hexToBytes(hex));
    }
    get x() {
    ...
    multiply(scalar) {
      ...
      let point, fake;
      const mul = /* @__PURE__ */ __name((n) => wnaf.cached(this, n, (p) => normalizeZ(Point3, p)), "mul");
      if (endo2) {
...
function ecdh(Point3, ecdhOpts = {}) {
  ...
  const lengths2 = Object.assign(getWLengths(Point3.Fp, Fn2), { seed: getMinHashLength(Fn2.ORDER) });
  function isValidSecretKey2(secretKey) {
    try {
  ...
  __name(isValidSecretKey2, "isValidSecretKey");
  function isValidPublicKey2(publicKey, isCompressed) {
  ...
  const utils3 = {
    isValidSecretKey: isValidSecretKey2,
    isValidPublicKey: isValidPublicKey2,
    randomSecretKey: randomSecretKey2
  };
  const keygen2 = createKeygen(randomSecretKey2, getPublicKey2);
  return Object.freeze({ getPublicKey: getPublicKey2, getSharedSecret, keygen: keygen2, Point: Point3, utils: utils3, lengths: lengths2 });
}
__name(ecdh, "ecdh");
function ecdsa(Point3, hash, ecdsaOpts = {}) {
  ...
  const { keygen: keygen2, getPublicKey: getPublicKey2, getSharedSecret, utils: utils3, lengths: lengths2 } = ecdh(Point3, ecdsaOpts);
  ...
  class Signature2 {
    ...
    recoverPublicKey(messageHash) {
      ...
      const R = Point3.fromBytes(concatBytes(pprefix((recovery & 1) === 0), x));
  ...
  const bits2int2 = ecdsaOpts.bits2int || /* @__PURE__ */ __name(function bits2int_def(bytes) {
  ...
  const bits2int_modN2 = ecdsaOpts.bits2int_modN || /* @__PURE__ */ __name(function bits2int_modN_def(bytes) {
...
var hash160 = /* @__PURE__ */ __name((data) => ripemd160(sha2562(data)), "hash160");
var fromU32 = /* @__PURE__ */ __name((data) => createView(data).getUint32(0, false), "fromU32");
var toU32 = /* @__PURE__ */ __name((n) => {
...
var HDKey = class _HDKey {
  ...
  deriveChild(index2) {
    ...
    const childTweak = I2.slice(0, 32);
    const chainCode = I2.slice(32);
...
var isBytes3 = /* @__PURE__ */ __name((a) => a instanceof Uint8Array || ArrayBuffer.isView(a) && a.constructor.name === "Uint8Array", "isBytes");
var abytes2 = /* @__PURE__ */ __name((value, length, title = "") => {
...
var bytesToHex2 = /* @__PURE__ */ __name((b) => Array.from(abytes2(b)).map((e) => padh(e, 2)).join(""), "bytesToHex");
...
var concatBytes2 = /* @__PURE__ */ __name((...arrs) => {
...
var randomBytes2 = /* @__PURE__ */ __name((len = L) => {
  const c = cr();
...
var invert2 = /* @__PURE__ */ __name((num, md) => {
  if (num === 0n || md <= 0n)
...
var apoint = /* @__PURE__ */ __name((p) => p instanceof Point2 ? p : err("Point expected"), "apoint");
...
Point2.BASE = G;
Point2.ZERO = I;
var bytesToNumBE = /* @__PURE__ */ __name((b) => big("0x" + (bytesToHex2(b) || "0")), "bytesToNumBE");
var sliceBytesNumBE = /* @__PURE__ */ __name((b, from, to) => bytesToNumBE(b.subarray(from, to)), "sliceBytesNumBE");
var B256 = 2n ** 256n;
var numTo32b = /* @__PURE__ */ __name((num) => hexToBytes2(padh(arange(num, 0n, B256), L2)), "numTo32b");
...
var isValidSecretKey = /* @__PURE__ */ __name((secretKey) => {
  try {
...
var isValidPublicKey = /* @__PURE__ */ __name((publicKey, isCompressed) => {
...
var hmacDrbg = /* @__PURE__ */ __name((seed, pred) => {
  let v = u8n(L);
  let k = u8n(L);
  let i = 0;
  const reset = /* @__PURE__ */ __name(() => {
  ...
  const h = /* @__PURE__ */ __name((...b) => callHash("hmacSha256")(k, concatBytes2(v, ...b)), "h");
  const reseed = /* @__PURE__ */ __name((seed2 = NULL) => {
    k = h(byte0, seed2);
    v = h();
    if (seed2.length === 0)
      return;
...
var _sign = /* @__PURE__ */ __name((messageHash, secretKey, opts, hmacDrbg2) => {
...
var sign = /* @__PURE__ */ __name((message, secretKey, opts = {}) => {
  opts = setDefaults(opts);
  message = prepMsg(message, opts, false);
  return _sign(message, secretKey, opts, hmacDrbg);
}, "sign");
var randomSecretKey = /* @__PURE__ */ __name((seed = randomBytes2(lengths.seed)) => {
  abytes2(seed);
...
var createKeygen2 = /* @__PURE__ */ __name((getPublicKey2) => (seed) => {
...
__name(generateWallet, "generateWallet");
function encryptData(data, secret) {
  const key = Buffer.from(sha2562(secret));
...
function createCoinbaseTransaction(minerAddress, reward, blockHeight) {
  const tx = {
    ...
    value: reward,
    nonce: blockHeight,
    // Use block height as nonce for coinbase
    gasLimit: 0,
    // No gas for coinbase
    gasPrice: "0",
    // No gas price for coinbase
    timestamp: Date.now()
  };
  ...
  return {
    ...
    hash,
    signature: "0x"
    // No signature for coinbase
  };
}
__name(createCoinbaseTransaction, "createCoinbaseTransaction");

// src/routes/mining.ts
var app3 = new Hono2();
var getMiningJobSchema = external_exports.object({
  minerAddress: external_exports.string().regex(/^0x[a-fA-F0-9]{40}$/)
});
var submitSolutionSchema = external_exports.object({
  jobId: external_exports.string(),
  nonce: external_exports.string(),
  minerAddress: external_exports.string().regex(/^0x[a-fA-F0-9]{40}$/)
});
app3.post("/job", zValidator("json", getMiningJobSchema), async (c) => {
  const db = c.get("db");
  const { minerAddress } = c.req.valid("json");
  try {
    const now = /* @__PURE__ */ new Date();
    await db.delete(miningJobs).where(lt(miningJobs.expiresAt, now));
    const [state] = await db.select().from(chainState).where(eq(chainState.id, 1)).limit(1);
    if (!state) {
      return c.json({ error: "Chain not initialized" }, 500);
    }
    const pendingTxs = await db.select().from(mempool).orderBy(desc(mempool.priority)).limit(100);
    ...
    const txHashes = [coinbaseTx.hash, ...txList.map((tx) => tx.hash)];
    const blockTemplate = {
    ...
    const expiresAt = new Date(Date.now() + 3e4);
...
app3.post("/submit", zValidator("json", submitSolutionSchema), async (c) => {
  const db = c.get("db");
  const { jobId, nonce, minerAddress } = c.req.valid("json");
  try {
    const [job] = await db.select().from(miningJobs).where(and(
    ...
    if (/* @__PURE__ */ new Date() > job.expiresAt) {
      return c.json({
        accepted: false,
        reason: "Job expired"
      }, 400);
    ...
    try {
      await db.update(miningJobs).set({ completed: true }).where(eq(miningJobs.jobId, jobId));
      let totalGasUsed = 0n;
      for (const tx of template.transactions.slice(1)) {
        totalGasUsed += BigInt(tx.gasUsed || 21e3);
      }
      const newBlock = {
        height: template.height,
        hash,
      ...
      for (const tx of template.transactions) {
        if (tx.from === "0x0000000000000000000000000000000000000000") {
          ...
          await db.insert(transactions).values({
            ...
            gasUsed: tx.gasUsed || 21e3,
            nonce: tx.nonce,
            signature: tx.signature,
            status: "confirmed"
          });
          const [senderWallet] = await db.select().from(wallets).where(eq(wallets.address, tx.from)).limit(1);
          if (senderWallet) {
            const cost = BigInt(tx.value) + BigInt(tx.gasUsed || 21e3) * BigInt(tx.gasPrice);
          ...
          const [recipientWallet] = await db.select().from(wallets).where(eq(wallets.address, tx.to)).limit(1);
          if (recipientWallet) {
            const newBalance = BigInt(recipientWallet.balance) + BigInt(tx.value);
            await db.update(wallets).set({ balance: newBalance.toString() }).where(eq(wallets.address, tx.to));
          } else {
        ...
        await db.delete(mempool).where(eq(mempool.txHash, tx.hash));
      }
      const [minerWallet] = await db.select().from(wallets).where(eq(wallets.address, minerAddress)).limit(1);
      if (minerWallet) {
        const newBalance = BigInt(minerWallet.balance) + BigInt(MINING_REWARD);
        await db.update(wallets).set({ balance: newBalance.toString() }).where(eq(wallets.address, minerAddress));
      } else {
        await db.insert(wallets).values({
      ...
      const [currentState] = await db.select().from(chainState).where(eq(chainState.id, 1)).limit(1);
      ...
      if (template.height >= nextAdjust) {
        const recentBlocks = await db.select().from(blocks).where(gte(blocks.height, template.height - 10)).orderBy(blocks.height);
        if (recentBlocks.length >= 10) {
          const timeSpan = recentBlocks[recentBlocks.length - 1].timestamp - recentBlocks[0].timestamp;
          const avgBlockTime = timeSpan * 1e3 / (recentBlocks.length - 1);
      ...
      await db.update(chainState).set({
...
app3.get("/stats", async (c) => {
  ...
  try {
    ...
    const activeJobs = await db.select({
      count: sql`count(*)`
    }).from(miningJobs).where(and(
      eq(miningJobs.completed, false),
      gte(miningJobs.expiresAt, /* @__PURE__ */ new Date())
    ));
    return c.json({
...
async function handleRpcRequest(db, request) {
...
__name(handleRpcRequest, "handleRpcRequest");
async function handleMethod(db, method, params) {
  switch (method) {
    // Network methods
    case "net_version":
      return "1337";
    ...
    case "web3_sha3":
      ...
      const hashBytes = sha2562(dataBytes);
    ...
    case "eth_blockNumber":
      const [latestBlock] = await db.select({ height: blocks.height }).from(blocks).orderBy(desc(blocks.height)).limit(1);
      return "0x" + (latestBlock?.height || 0).toString(16);
    case "eth_getBalance":
      const address = params[0].toLowerCase();
      const blockParam = params[1] || "latest";
      const [wallet] = await db.select({ balance: wallets.balance }).from(wallets).where(eq(wallets.address, address));
      if (!wallet) {
        return "0x0";
      }
    ...
    case "eth_getTransactionCount":
      ...
      const txCount = await db.select({ count: sql`count(*)` }).from(transactions).where(eq(transactions.from, addr));
      return "0x" + (txCount[0]?.count || 0).toString(16);
    case "eth_getBlockByNumber":
      const blockNumber = parseInt(params[0], 16);
      const fullTx = params[1] || false;
      const [block] = await db.select().from(blocks).where(eq(blocks.height, blockNumber));
      if (!block) {
        return null;
      }
    ...
    case "eth_getBlockByHash":
      ...
      const [blockByHash] = await db.select().from(blocks).where(eq(blocks.hash, blockHash));
      if (!blockByHash) {
        return null;
      }
    ...
    case "eth_getTransactionByHash":
      ...
      const [tx] = await db.select().from(transactions).where(eq(transactions.hash, txHash));
      if (!tx) {
        return null;
      }
    ...
    case "eth_getTransactionReceipt":
      ...
      const [transaction] = await db.select({
        tx: transactions,
        block: blocks
      }).from(transactions).leftJoin(blocks, eq(transactions.blockHash, blocks.hash)).where(eq(transactions.hash, receiptHash));
      if (!transaction?.tx) {
        return null;
      }
    ...
    case "eth_sendTransaction":
      ...
      const [sender] = await db.select().from(wallets).where(eq(wallets.address, from));
    ...
    case "eth_call":
      ...
      if (callParams.data && callParams.data.startsWith("0x70a08231")) {
        ...
        const [bal] = await db.select({ balance: wallets.balance }).from(wallets).where(eq(wallets.address, addrParam.toLowerCase()));
    ...
    case "eth_accounts":
      return [];
    case "eth_coinbase":
      const [bot] = await db.select({ address: wallets.address }).from(wallets).where(eq(wallets.type, "bot"));
...
__name(handleMethod, "handleMethod");
function formatBlock(block, includeTx) {
  return {
    ...
    // TODO: include full transactions if requested
    uncles: []
  };
}
...
function formatReceipt(tx, block) {
  return {
...
__name(formatReceipt, "formatReceipt");
...
function generateTxId() {
  ...
  const hashBytes = sha2562(encoder.encode(data));
  return "0x" + bytesToHex(hashBytes);
}
...
// src/services/bot.ts
var BlockchainBot = class {
  static {
    __name(this, "BlockchainBot");
  }
  db;
  config;
  encryptionSecret;
  ...
  async createGenesisBlock(botAddress) {
  ...
  async sendRandomTransaction() {
    ...
    try {
      ...
      const [botWallet] = await this.db.select().from(wallets).where(eq(wallets.address, this.config.address)).limit(1);
      if (!botWallet) {
      ...
      const minAmount = STARS_TO_STARSHARS / 10n;
      const maxAmount = STARS_TO_STARSHARS * 10n;
      ...
      await this.db.update(wallets).set({ nonce: botWallet.nonce + 1 }).where(eq(wallets.address, this.config.address));
      await this.db.update(botConfig).set({
        txCount: this.config.txCount + 1,
        lastTxAt: /* @__PURE__ */ new Date()
  ...
  async startTransactionLoop() {
    const sendTransaction = /* @__PURE__ */ __name(async () => {
  ...
  async getStats() {
    ...
    const [stats] = await this.db.select().from(botConfig).where(eq(botConfig.id, 1)).limit(1);
    return stats || null;
  }
};
...
app4.route("/api/blockchain", blockchain_default);
app4.route("/api/transactions", transactions_default);
app4.route("/api/mining", mining_default);
app4.route("/rpc", rpc_default);
var botInitialized = false;
app4.use("/api/*", async (c, next) => {
  if (!botInitialized) {
    botInitialized = true;
    const db = c.get("db");
  ...
  return c.json(
    {
      ...
      message: err2.message || "An unexpected error occurred"
    },
    500
  );
});
...
// .wrangler/tmp/bundle-x6F5Yi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-x6F5Yi/middleware-loader.entry.ts
