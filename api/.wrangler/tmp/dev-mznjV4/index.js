...
// .wrangler/tmp/bundle-dZMVEe/checked-fetch.js
...
// .wrangler/tmp/bundle-dZMVEe/strip-cf-connecting-ip-header.js
...
_a49 = entityKind;
__publicField(SQLiteRealBuilder, _a49, "SQLiteRealBuilder");
var _a50;
...
_a50 = entityKind;
__publicField(SQLiteReal, _a50, "SQLiteReal");
function real(name) {
...
_a51 = entityKind;
__publicField(SQLiteTextBuilder, _a51, "SQLiteTextBuilder");
var _a52;
...
_a52 = entityKind;
__publicField(SQLiteText, _a52, "SQLiteText");
var _a53;
...
_a53 = entityKind;
__publicField(SQLiteTextJsonBuilder, _a53, "SQLiteTextJsonBuilder");
var _a54;
...
_a54 = entityKind;
__publicField(SQLiteTextJson, _a54, "SQLiteTextJson");
...
_a55 = entityKind;
__publicField(SQLiteViewBase, _a55, "SQLiteViewBase");
...
_a56 = entityKind;
__publicField(SQLiteDialect, _a56, "SQLiteDialect");
var _a57;
...
_a57 = entityKind;
__publicField(SQLiteSyncDialect, _a57, "SQLiteSyncDialect");
var _a58;
...
_a58 = entityKind;
__publicField(SQLiteAsyncDialect, _a58, "SQLiteAsyncDialect");
...
_a60 = entityKind;
__publicField(SQLiteSelectBuilder, _a60, "SQLiteSelectBuilder");
var _a61;
...
_a61 = entityKind;
__publicField(SQLiteSelectQueryBuilderBase, _a61, "SQLiteSelectQueryBuilder");
var _a62;
...
_a62 = entityKind;
__publicField(SQLiteSelectBase, _a62, "SQLiteSelect");
...
_a63 = entityKind;
__publicField(QueryBuilder, _a63, "SQLiteQueryBuilder");
...
_a64 = entityKind;
__publicField(SQLiteUpdateBuilder, _a64, "SQLiteUpdateBuilder");
var _a65;
...
_a65 = entityKind;
__publicField(SQLiteUpdateBase, _a65, "SQLiteUpdate");
...
_a66 = entityKind;
__publicField(RelationalQueryBuilder, _a66, "SQLiteAsyncRelationalQueryBuilder");
var _a67;
...
_a67 = entityKind;
__publicField(SQLiteRelationalQuery, _a67, "SQLiteAsyncRelationalQuery");
var _a68;
...
_a68 = entityKind;
__publicField(SQLiteSyncRelationalQuery, _a68, "SQLiteSyncRelationalQuery");
...
_a69 = entityKind;
__publicField(SQLiteRaw, _a69, "SQLiteRaw");
...
_a70 = entityKind;
__publicField(BaseSQLiteDatabase, _a70, "BaseSQLiteDatabase");
...
var interactions = sqliteTable("interactions", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var miningStats = sqliteTable("mining_stats", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  sessionId: text("session_id").notNull(),
...
var chainForks = sqliteTable("chain_forks", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var createForkSchema = external_exports.object({
  forkId: external_exports.string().min(1),
  parentChainId: external_exports.string().min(1),
  forkHeight: external_exports.number().int().min(0),
  isMainChain: external_exports.boolean().optional()
});
...
app.post("/blocks", zValidator("json", createBlockSchema), async (c) => {
  ...
  try {
    ...
    if (data.chainId) {
      await db.update(chainForks).set({
...
app.get("/forks", async (c) => {
  const db = c.get("db");
  try {
    const forks = await db.select().from(chainForks).orderBy(desc(chainForks.createdAt));
    return c.json(forks);
  } catch (error) {
...
app.post("/forks", zValidator("json", createForkSchema), async (c) => {
  ...
  try {
    ...
    const [created] = await db.insert(chainForks).values(newFork).returning();
    return c.json(created, 201);
  } catch (error) {
...
app.put("/forks/:forkId/resolve", async (c) => {
  ...
  try {
    await db.update(chainForks).set({ isMainChain: false });
    const [resolved] = await db.update(chainForks).set({
      isMainChain: true,
      resolvedAt: /* @__PURE__ */ new Date()
    }).where(eq(chainForks.forkId, forkId)).returning();
    if (!resolved) {
...
app2.get("/", async (c) => {
  ...
  try {
    ...
    const result = await query.orderBy(desc(transactions.createdAt)).limit(limit).offset(offset);
    return c.json({
      transactions: result,
      count: result.length,
      limit,
...
app2.get("/mempool/pending", async (c) => {
  const db = c.get("db");
  try {
    const pendingTxs = await db.select().from(transactions).where(eq(transactions.status, "pending")).orderBy(desc(transactions.createdAt)).limit(50);
    return c.json({
      mempool: pendingTxs,
...
var app3 = new Hono2();
var recordInteractionSchema = external_exports.object({
  sessionId: external_exports.string().min(1),
  type: external_exports.enum(["click", "transaction", "mining_boost"]),
  data: external_exports.string().optional(),
  positionX: external_exports.number().optional(),
  positionY: external_exports.number().optional()
});
var updateMiningStatsSchema = external_exports.object({
  sessionId: external_exports.string().min(1),
  speedMultiplier: external_exports.number().min(1),
  blocksMinedCount: external_exports.number().int().min(0).optional(),
  totalClicks: external_exports.number().int().min(0).optional(),
  averageMiningTime: external_exports.number().optional(),
  peakSpeedMultiplier: external_exports.number().min(1).optional()
});
app3.post("/interactions", zValidator("json", recordInteractionSchema), async (c) => {
  const db = c.get("db");
  const data = c.req.valid("json");
  try {
    ...
    const [created] = await db.insert(interactions).values(newInteraction).returning();
    return c.json(created, 201);
  } catch (error) {
...
app3.get("/interactions/:sessionId", async (c) => {
  const db = c.get("db");
  const sessionId = c.req.param("sessionId");
  const limit = Number(c.req.query("limit")) || 100;
  try {
    const sessionInteractions = await db.select().from(interactions).where(eq(interactions.sessionId, sessionId)).orderBy(desc(interactions.timestamp)).limit(limit);
    return c.json({
      sessionId,
...
app3.post("/mining-stats", zValidator("json", updateMiningStatsSchema), async (c) => {
  const db = c.get("db");
  const data = c.req.valid("json");
  try {
    const [existing] = await db.select().from(miningStats).where(eq(miningStats.sessionId, data.sessionId)).limit(1);
    if (existing) {
      const updates = {
        speedMultiplier: data.speedMultiplier,
        timestamp: /* @__PURE__ */ new Date()
      };
      if (data.blocksMinedCount !== void 0) {
        updates.blocksMinedCount = data.blocksMinedCount;
      }
      if (data.totalClicks !== void 0) {
        updates.totalClicks = data.totalClicks;
      }
      if (data.averageMiningTime !== void 0) {
        updates.averageMiningTime = data.averageMiningTime;
      }
      if (data.peakSpeedMultiplier !== void 0 && data.peakSpeedMultiplier > existing.peakSpeedMultiplier) {
        updates.peakSpeedMultiplier = data.peakSpeedMultiplier;
      }
      const [updated] = await db.update(miningStats).set(updates).where(eq(miningStats.sessionId, data.sessionId)).returning();
      return c.json(updated);
    } else {
      const newStats = {
        ...data,
      ...
      const [created] = await db.insert(miningStats).values(newStats).returning();
      return c.json(created, 201);
    }
  } catch (error) {
...
app3.get("/mining-stats/:sessionId", async (c) => {
  const db = c.get("db");
  const sessionId = c.req.param("sessionId");
  try {
    const [stats] = await db.select().from(miningStats).where(eq(miningStats.sessionId, sessionId)).limit(1);
    if (!stats) {
...
app3.get("/global", async (c) => {
  const db = c.get("db");
  const hoursAgo = Number(c.req.query("hoursAgo")) || 24;
  try {
    const timeThreshold = new Date(Date.now() - hoursAgo * 60 * 60 * 1e3);
    const interactionStats = await db.select({
      totalInteractions: sql`count(*)`,
      uniqueSessions: sql`count(distinct session_id)`,
      clickCount: sql`sum(case when type = 'click' then 1 else 0 end)`,
      transactionCount: sql`sum(case when type = 'transaction' then 1 else 0 end)`,
      miningBoostCount: sql`sum(case when type = 'mining_boost' then 1 else 0 end)`
    }).from(interactions).where(gte(interactions.timestamp, timeThreshold));
    const miningPerformance = await db.select({
      avgSpeedMultiplier: sql`avg(speed_multiplier)`,
      maxSpeedMultiplier: sql`max(peak_speed_multiplier)`,
      totalBlocksMined: sql`sum(blocks_mined_count)`,
      totalClicks: sql`sum(total_clicks)`,
      avgMiningTime: sql`avg(average_mining_time)`,
      activeSessions: sql`count(*)`
    }).from(miningStats).where(gte(miningStats.timestamp, timeThreshold));
    const recentActivity = await db.select({
      hour: sql`strftime('%Y-%m-%d %H:00', timestamp)`,
      interactions: sql`count(*)`
    }).from(interactions).where(gte(interactions.timestamp, timeThreshold)).groupBy(sql`strftime('%Y-%m-%d %H:00', timestamp)`).orderBy(desc(sql`strftime('%Y-%m-%d %H:00', timestamp)`));
    return c.json({
      timeRange: {
        hours: hoursAgo,
...
var analytics_default = app3;

// src/routes/nodes.ts
var app4 = new Hono2();
var createNodeSchema = external_exports.object({
  nodeId: external_exports.string().min(1),
  type: external_exports.enum(["validator", "miner", "peer", "smart-contract"]),
  address: external_exports.string().min(1),
  isActive: external_exports.boolean().optional()
});
var updateNodeSchema = external_exports.object({
  isActive: external_exports.boolean().optional(),
  consensusParticipation: external_exports.number().int().optional()
});
app4.get("/", async (c) => {
  const db = c.get("db");
  ...
  try {
    ...
    if (isActive !== void 0) {
      query = query.where(eq(nodes.isActive, isActive));
    }
...
app4.get("/:nodeId", async (c) => {
  const db = c.get("db");
  const nodeId = c.req.param("nodeId");
  try {
    const [node] = await db.select().from(nodes).where(eq(nodes.nodeId, nodeId)).limit(1);
    if (!node) {
...
app4.post("/", zValidator("json", createNodeSchema), async (c) => {
  const db = c.get("db");
  const data = c.req.valid("json");
  try {
    const newNode = {
    ...
    const [created] = await db.insert(nodes).values(newNode).returning();
    return c.json(created, 201);
  } catch (error) {
...
app4.patch("/:nodeId", zValidator("json", updateNodeSchema), async (c) => {
  const db = c.get("db");
  const nodeId = c.req.param("nodeId");
  const data = c.req.valid("json");
  try {
    ...
    const [updated] = await db.update(nodes).set(updates).where(eq(nodes.nodeId, nodeId)).returning();
    if (!updated) {
      return c.json({ error: "Node not found" }, 404);
    }
    return c.json(updated);
...
app4.post("/:nodeId/heartbeat", async (c) => {
  const db = c.get("db");
  const nodeId = c.req.param("nodeId");
  try {
    const [updated] = await db.update(nodes).set({
      lastSeen: /* @__PURE__ */ new Date(),
      isActive: true
    }).where(eq(nodes.nodeId, nodeId)).returning();
    if (!updated) {
      return c.json({ error: "Node not found" }, 404);
    }
    return c.json({
...
app4.get("/stats/network", async (c) => {
  const db = c.get("db");
  try {
    const stats = await db.select({
      totalNodes: sql`count(*)`,
      activeNodes: sql`sum(case when is_active = true then 1 else 0 end)`,
      validators: sql`sum(case when type = 'validator' then 1 else 0 end)`,
      miners: sql`sum(case when type = 'miner' then 1 else 0 end)`,
      peers: sql`sum(case when type = 'peer' then 1 else 0 end)`,
      smartContracts: sql`sum(case when type = 'smart-contract' then 1 else 0 end)`,
      totalConsensusParticipation: sql`sum(consensus_participation)`
    }).from(nodes);
    const topValidators = await db.select().from(nodes).where(eq(nodes.type, "validator")).orderBy(desc(nodes.consensusParticipation)).limit(5);
    return c.json({
      network: stats[0] || {},
      topValidators,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
...
app4.post("/maintenance/deactivate-inactive", async (c) => {
  const db = c.get("db");
  const hoursInactive = Number(c.req.query("hours")) || 24;
  try {
    const threshold = new Date(Date.now() - hoursInactive * 60 * 60 * 1e3);
    const result = await db.update(nodes).set({ isActive: false }).where(
...
app5.route("/api/blockchain", blockchain_default);
app5.route("/api/transactions", transactions_default);
app5.route("/api/analytics", analytics_default);
app5.route("/api/nodes", nodes_default);
app5.notFound((c) => {
  return c.json(
    {
      error: "Not Found",
...
// .wrangler/tmp/bundle-dZMVEe/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-dZMVEe/middleware-loader.entry.ts
