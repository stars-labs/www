<script lang="ts">
  ...
  function displayMiner(minerAddress: string | undefined): string {
    if (!minerAddress) return '';
    if (minerAddress === myMinerAddress) return 'You';
    return formatHash(minerAddress);
  }
          ...
          <table class="w-full">
            ...
            <tbody>
              ...
              {#each blocks as block}
                <tr class="border-b border-gray-800 hover:bg-gray-800/30 transition cursor-pointer {block.minerAddress === myMinerAddress ? 'bg-yellow-500/10' : ''}"
                  ...
                  <td class="px-4 py-3 font-mono text-xs {block.minerAddress === myMinerAddress ? 'text-yellow-300 font-bold' : ''}">{displayMiner(block.minerAddress)}</td>
